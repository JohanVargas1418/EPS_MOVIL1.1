...
export default function EditarCitasScreen() {
  ...
  const cita = route.params?.cita; // Obtiene la cita desde los parámetros de la ruta // Estados para los campos del formulario
  ...
  const [observacion, setObservacion] = useState(
    cita?.observacion?.toString() || ""
  );
  const [tipo_consulta, setTipo_consulta] = useState(
    cita?.tipo_consulta?.toString() || ""
  );
  const [loading, setLoading] = useState(false); // Estado para controlar el loading
  const esEdicion = !!cita; // Determina si es una edición o una nueva cita // Función para manejar el guardado de la cita
  const handleGuardar = async () => {
    ...
    const horaValida = /^([01]\d|2[0-3]):([0-5]\d)$/.test(hora); // Formato HH:MM
    if (!horaValida) {
      Alert.alert("Error", "La hora debe estar en el formato HH:MM");
      return;
    }

    setLoading(true); // Activa el loading
    try {
      let result; // Llama a la función de editar o crear según corresponda
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.formContainer}>
          ...
          <Text style={styles.formTitle}>
            {esEdicion ? "Editar Detalles de Cita" : "Crear Nueva Cita"}
          </Text>
                   {" "}
...
const styles = StyleSheet.create({
  ...
  }, // El BottonComponent debería manejar su propio estilo de botón y texto. // Estos estilos son solo de referencia si BottonComponent no los provee.
  buttonText: {
    color: "white",
    fontSize: 16,
    fontWeight: "bold",
